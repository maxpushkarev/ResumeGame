Shader "MyOwnShaders/HalfLambertRampTextureWithViewDir" {
	Properties {
	...
	SubShader {
		...
		half4 LightingHalfLambert(SurfaceOutput s, fixed3 lightDir, half3 viewDir, fixed atten)
		{
			...
			float3 ramp = tex2D(_RampText, float2(halfDifLight, rimLight)).rgb;
